...
namespace Lesson10
{
    ...
    class Program
    {
        ...
        static void Main()
        {
            ...
            var result4 = debtors.Where(d => (d.FullName.Length>18 && d.Phone.Count(i => i =='7')>=2));
                

            //5) Qishda anadan olan borclulari cixardmaq

            var result5 = debtors.Where(d => d.BirthDay.Month==12 ||  d.BirthDay.Month<=2)
                .Select(d=>d.BirthDay) ;
            ...
            var result6 = debtors.Where(d => d.Debt > debtors.Average(s => s.Debt))
                .OrderBy(debtors => debtors.FullName.Split().Last())
            ...
            var result7 = debtors.Where(d => !d.Phone.Contains('8'))
                .Select(d => d.FullName.Split().Last() + " " + (DateTime.Now.Year-d.BirthDay.Year)+ " "+ d.Debt);
            ...
            var result8 = debtors.Where(d => (d.FullName.Replace(" ", "").ToLower().Distinct().Count()) >= 3)
